...
class UploadFinalizeController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        try {
            ...
            /* Scan (degraded allowed) */
            // Scanner verdict is an outcome that influences publishability,
            // but must never block finalization.
            $scanStatus = 'pending_scan';
                ...
                if ($scannerResponse->ok()) {
                    ...
                    if (is_array($body) && ($body['status'] ?? null) === 'infected') {
                        ...
                        $this->safeEmit(
                            ...
                            $uploadId,
                            'api',
                            ['verdict' => 'infected', 'signature' => $body['signature'] ?? null]
                        );

                        $scanVerdict = 'infected';
                        $scanSignature = $body['signature'] ?? null;
                        $scanStatus = 'infected';
                    }
    ...
    private function mapFailureReason(\Throwable $e): string
    {
        ...
        // Scanner interaction failures are always normalized
        // to scanner_unavailable (scanner availability must not
        // influence finalize determinism).
        if (
            $msg === 'scan_timeout' ||
